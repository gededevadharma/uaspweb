<div class="d-flex justify-content-between align-items-center mb-3">
    <a href="<?= base_url('admin/peminjaman/tambah') ?>" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Tambah Peminjaman</a>
    <div class="d-flex gap-2">
        <a href="<?= base_url('admin/peminjaman') ?>" class="btn btn-outline-dark btn-sm <?= !$status_filter ? 'active' : '' ?>">Semua</a>
        <a href="<?= base_url('admin/peminjaman?status=dipinjam') ?>" class="btn btn-outline-warning btn-sm <?= $status_filter == 'dipinjam' ? 'active' : '' ?>">Dipinjam</a>
        <a href="<?= base_url('admin/peminjaman?status=dikembalikan') ?>" class="btn btn-outline-success btn-sm <?= $status_filter == 'dikembalikan' ? 'active' : '' ?>">Dikembalikan</a>
        <a href="<?= base_url('admin/peminjaman?status=terlambat') ?>" class="btn btn-outline-danger btn-sm <?= $status_filter == 'terlambat' ? 'active' : '' ?>">Terlambat</a>
        <form action="<?= base_url('admin/peminjaman') ?>" method="get" class="d-flex ms-2">
            ...
            <a href="<?= base_url('admin/peminjaman') ?>" class="btn btn-outline-danger ms-1"><i class="bi bi-x-circle"></i></a>
            <?php endif; ?>
        </form>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <nav><ul class="pagination justify-content-center">
            ...
            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="<?= base_url('admin/peminjaman?page=' . $i . ($search ? '&search=' . $search : '') . ($status_filter ? '&status=' . $status_filter : '')) ?>"><?= $i ?></a>
            </li>
            <?php endfor; ?>
        </ul></nav>
        <?php endif; ?>
        ...
        <div class="text-center py-5">
            ...
            <a href="<?= base_url('admin/peminjaman/tambah') ?>" class="btn btn-primary mt-2">Tambah Peminjaman</a>
        </div>
        <?php endif; ?>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (modal) {
        modal.addEventListener('show.bs.modal', function(event) {
            ...
            document.getElementById('kembalikan-link').href = '<?= base_url('admin/peminjaman/kembali/') ?>' + btn.getAttribute('data-id');
        });
    }
});
</script>
