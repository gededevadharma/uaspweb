<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <form action="<?= base_url('peminjam') ?>" method="get" class="d-flex">
        <input type="text" name="search" class="form-control search-box me-2" placeholder="Cari nama/no HP..." value="<?= $search ?>">
        <button class="btn btn-outline-dark" type="submit"><i class="bi bi-search"></i></button>
        <?php if ($search): ?>
        <a href="<?= base_url('peminjam') ?>" class="btn btn-outline-danger ms-1"><i class="bi bi-x-circle"></i></a>
        <?php endif; ?>
    </form>
</div>
    ...
    <div class="card-body">
        ...
        <nav><ul class="pagination justify-content-center">
            ...
            <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="<?= base_url('peminjam?page=' . $i . ($search ? '&search=' . $search : '')) ?>"><?= $i ?></a>
            </li>
            <?php endfor; ?>
        </ul></nav>
        <?php endif; ?>
